...
class Flow(BaseNode):
    ...
    def _orch(self,shared,params=None):
        curr, p, last_exec_res = copy.copy(self.start_node), (params or {**self.params}), None
        while curr:
            curr.set_params(p)
            prep_res = curr.prep(shared)
            
            # The input to exec is the result of the previous node's execution
            # or the result of the current node's prep function.
            exec_input = last_exec_res if last_exec_res is not None else prep_res
            
            last_exec_res = curr._exec(exec_input)
            
            last_action = curr.post(shared, prep_res, last_exec_res)
